...
The function prints the `hosp_id` of each hospital for which diversion is calculated. The resulting matrix shows diversions from each of the merging hospitals to all other hospitals. The diversions should sum to 1. (If the diversion does not sum to 1, that means that a degenerate cell exists and a notification flag will be printed.) We can verify that our diversions sum to one for Hospital 1:
